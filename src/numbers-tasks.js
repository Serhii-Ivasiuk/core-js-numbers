...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const firstLegLength = Math.abs(x1) + Math.abs(x2);
  const secondLegLength = Math.abs(y1) + Math.abs(y2);

  return Math.sqrt(firstLegLength ** 2 + secondLegLength ** 2);
}
...
function getParallelepipedDiagonal(a, b, c) {
  // d ** 2 = a ** 2 + b ** 2 + c ** 2;
  const sumOfSquaresOfSides = a ** 2 + b ** 2 + c ** 2;

  return Math.sqrt(sumOfSquaresOfSides);
}
